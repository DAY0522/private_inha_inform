...
int main()
{
	...
	Account account2(25, "account2"); // Account형인 account2 변수를 생성하고, 생성자를 이용해 초기 잔고를 $25, 계좌명을 account2로 각 멤버 변수를 초기화 한다.

	cout << "account1 balance : $" << account1.getBalance() << endl; // getBalance 함수의 return 값을 출력해 account1의 초기 잔고를 확인한다.
	cout << "account2 balance : $" << account2.getBalance() << endl; // getBalance 함수의 return 값을 출력해 account2의 초기 잔고를 확인한다.

	account1.debit(); // debit 함수를 이용해 acccount1 계좌에 출금을 한다.

	cout << "account1 balance : $" << account1.getBalance() << endl; // 출금이 잘 됐는지 확인하기 위해 getBalance 함수의 return 값을 다시 출력해 입금 후의 account1의 잔고를 확인한다.
	cout << "account2 balance : $" << account2.getBalance() << endl; // 출금이 잘 됐는지 확인하기 위해 getBalance 함수의 return 값을 다시 출력해 입금 후의 account2의 잔고를 확인한다.

	account2.debit(); // debit 함수를 이용해 acccount2 계좌에 출금을 한다.

	cout << "account1 balance : $" << account1.getBalance() << endl; // 출금이 잘 됐는지 확인하기 위해 getBalance 함수의 return 값을 다시 출력해 입금 후의 account1의 잔고를 확인한다.
	cout << "account2 balance : $" << account2.getBalance() << endl; // 출금이 잘 됐는지 확인하기 위해 getBalance 함수의 return 값을 다시 출력해 입금 후의 account1의 잔고를 확인한다.
